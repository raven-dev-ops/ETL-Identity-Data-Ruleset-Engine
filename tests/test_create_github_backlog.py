...
def test_parse_backlog_active_catalog() -> None:
    ...
    assert parsed.milestones == ("v1.0.0", "v1.1.0", "v1.2.0")
    assert len(parsed.labels) == 21
    assert len(parsed.epics) == 3
    assert len(parsed.issues) == 12
    assert parsed.issues[0].catalog_number == 105
    assert parsed.issues[-1].catalog_number == 116
    assert {issue.status for issue in parsed.issues} == {"closed", "open"}
    assert [issue.catalog_number for issue in parsed.issues if issue.status == "closed"] == [105, 106, 107, 108, 109]
...
def test_select_sync_backlog_skips_closed_catalog_entries_by_default() -> None:
    ...
    assert sync_backlog.milestones == ("v1.1.0", "v1.2.0")
