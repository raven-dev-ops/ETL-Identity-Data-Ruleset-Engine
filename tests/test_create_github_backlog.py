...
def test_parse_backlog_active_catalog() -> None:
    ...
    assert parsed.milestones == ("v0.7.0", "v0.8.0", "v0.9.0")
    assert len(parsed.labels) == 21
    assert len(parsed.epics) == 3
    assert len(parsed.issues) == 12
    assert parsed.issues[0].catalog_number == 93
    assert parsed.issues[-1].catalog_number == 104
    assert {issue.status for issue in parsed.issues} == {"open", "closed"}
    ...
    assert parsed.issues[11].status == "open"
...
def test_select_sync_backlog_skips_closed_catalog_entries_by_default() -> None:
    ...
    assert sync_backlog.issues[0].catalog_number == 104
