...
SUPPORTED_LANDING_ZONE_KIND = "local_filesystem"
...
class ResolvedBatchManifest:
    ...
    def input_paths(self) -> tuple[Path, ...]:
        return tuple(source.resolved_path for source in self.sources)
...
def _load_batch_manifest(path: Path) -> BatchManifest:
    ...
    if landing_zone_kind != SUPPORTED_LANDING_ZONE_KIND:
        raise _manifest_error(
            path,
            f"landing_zone.kind must be {SUPPORTED_LANDING_ZONE_KIND!r}",
        )
    landing_zone_base_path = Path(
    ...
    return BatchManifest(
        ...
        landing_zone=LandingZoneSpec(
            ...
            base_path=landing_zone_base_path,
        ),
        sources=tuple(sources),
    )


def _resolve_landing_zone_base_path(manifest_path: Path, landing_zone: LandingZoneSpec) -> Path:
    base_path = landing_zone.base_path
...
def _resolve_source_path(
    ...
    if source_path.is_absolute():
        ...
        resolved_path = _resolve_landing_zone_base_path(manifest_path, landing_zone) / source_path
