...
| Manual review queue | `data/review_queue/manual_review_queue.csv` | `review_id`, `left_id`, `right_id`, `score`, `reason_codes`, `top_contributing_match_signals`, `queue_status` | Fresh queue rows default `queue_status` to `pending`. When SQLite persistence is enabled, this file remains the portable artifact while richer lifecycle state lives in persisted `review_cases`. |
